...
// declare array of locks for each list in hash table
pthread_mutex_t locks[16384];
...
void* thread_process(void *para) {
    ...
    for (i = p->forloop_start; i < p->forloop_end; i++) {
        ...
        for (j = 0; j < SAMPLES_TO_COLLECT; j++) {
            ...
            //critical section
             pthread_mutex_lock(&locks[h.get_hashvalue(key)]);
            ...
            s->count++;
            pthread_mutex_unlock(&locks[h.get_hashvalue(key)]);
        }
    }
